...
#Definition of starting and ending dates for the construction of the file name
end_date='2025-01-29' 
...
BASE_URL_releases = "https://www.gamespot.com/api/releases/?api_key=" + API_KEY + "&format=json&filter=release_date:" + start_date + "|" + end_date + "&field_list=name,release_date"
...
response_releases= requests.get(BASE_URL_releases, headers=params2_releases).json()

while response_releases['number_of_total_results'] != response_releases['number_of_page_results'] :
#    last_result_date = 
    #print(type(response['number_of_total_results']))
    last_result_date = response_releases['results'][-1] #dernier date avec timeframe
    time_new= str(datetime.strptime((re.search(regex_date_USA, str(response_releases['results'][-1]))[0]), "%Y-%m-%d %H:%M:%S") + + timedelta(seconds=1))
    BASE_URL_new = str("https://www.gamespot.com/api/releases/?api_key=" + API_KEY + "&format=json&filter=release_date:"+ time_new + "|" + end_date + "&field_list=release_date,name&lim=1000000")
    response_releases= requests.get(BASE_URL_new, headers=params2_releases).json() #response object decoded into json(via requests method decoded of json)
    with open(str("C:\\Users\\samue\\OneDrive\\Bureau\\résumé_releases" + date_name_file + ".txt"), 'a') as f :
        for i in response_releases['results']:
            f.write(i.get('release_date') + i.get('name') + '\n') 
